...
class TestCodeExtraction:
    ...
    def test_extract_code_samples_session_280(self):
        ...
        # Load test HTML
        test_file = (
            Path(__file__).parent / "280-code-along-cook-up-a-rich-text-experience.html"
        )
        ...
        assert len(code_samples) >= 20, (
            f"Expected at least 20 code samples, got {len(code_samples)}"
        )
        ...
        basics_sample = next(
            (s for s in code_samples if "initial attempt" in s["title"]), None
        )
        assert basics_sample is not None
        assert (
            basics_sample["title"] == "Build custom controls: Basics (initial attempt)"
        )
        ...
        for sample in code_samples:
            assert sample["timestamp"].isdigit(), (
                f"Timestamp should be numeric: {sample['timestamp']}"
            )
    ...
    def test_format_code_samples_in_markdown(self):
        ...
        transcript_start = next(
            i for i, line in enumerate(lines) if line == "## Transcript"
        )
        ...
        assert (
            "### Code Sample: TextEditor and String - [1:15]" in transcript_section
        ), "First code sample not found"
        assert (
            "### Code Sample: AttributedString Basics - [4:43]" in transcript_section
        ), "Second code sample not found"
...
class TestTopicMapping:
    ...
    def test_sanitize_filename(self):
        ...
        test_cases = [
            ...
            (
                "Code-along: Cook up a rich experience!",
                "code-along-cook-up-a-rich-experience",
            ),
            ...
            (
                "Very " + "long " * 30 + "filename",
                "very" + "-long" * 19,
            ),  # Will truncate at 100 chars
        ]

        for input_str, expected in test_cases:
            result = downloader._sanitize_filename(input_str)
            assert result == expected, (
                f"Expected '{expected}' for '{input_str}', got '{result}'"
            )
            # Ensure it's valid for filesystem
            assert not any(c in result for c in '<>:"/\\|?*'), (
                f"Invalid chars in result: {result}"
            )
